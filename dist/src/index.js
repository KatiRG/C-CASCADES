(function () {
	...
	function makeStackedBar(chartId, fname, h, w) {
	  ...
	  d3.csv(fname, function (error, data) {
	    ...
	    var country = svg.selectAll(".country").data(data).enter().append("g").attr("class", "g").attr("transform", function () {
	    ...
	    country.selectAll("rect").on("mousemove", function (d) {
	      ...
	      div.html("<b>" + d.loac + "</b>" + "<br><br>" + "<table>" + "<tr>" + "<td><b>" + format(delta) + "</td>" + "<td>" + " " + units + "</td>" + "</tr>" + "</table>").style("left", d3.event.pageX + "px").style("top", d3.event.pageY - yshiftTooltip + "px");
	...
	function makeSankey(chartDiv, jsonFile, chartNum) {
	  ...
	  function make(graph) {
	    ...
	    link.on("mousemove", function (d) {
	      ...
	      var sourceName = d.source.name === "Tropics" || d.source.name === "Mid lat" || d.source.name === "High lat" ? nameDict[d.source.name] : d.source.name;
	      div.transition().style("opacity", .9);
	    ...
	    node.append("text").attr("x", -6).attr("y", function (d) {
	      ...
	      if (d.name === "Tropics" || d.name === "Mid lat" || d.name === "High lat") return nameDict[d.name];else return d.name;
	    ...
	    node.on("mousemove", function (d) {
	      ...
	      var sourceName = d.name === "Tropics" || d.name === "Mid lat" || d.name === "High lat" ? nameDict[d.name] : d.name;
	      div.transition().style("opacity", .9);
